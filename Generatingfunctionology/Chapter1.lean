...
-- -/
-- abbrev invOneScaled (a : R) : R⸨X⸩ := mk fun n => a^n
--
-- abbrev extractInvOneScaled (x : R⸨X⸩) : R⸨X⸩ := invOneScaled $ -(x.coeff 1)
--
-- @[simp]
-- lemma extractInv_unfold (x : R⸨X⸩) : extractInvOneScaled x = invOneScaled (-(x.coeff 1)) := by
--   simp
--
...
open HahnSeries (single)

namespace LaurentSeries

-- REMOVEME: after leanprover-community/mathlib4#16639
scoped notation:9000 R "⸨X⸩" => LaurentSeries R

variable {α : Type*} [Semiring α] (φ φ' : α⸨X⸩)

abbrev mk (f : ℕ → α) : α⸨X⸩ := ↑(PowerSeries.mk f)

section 

open PowerSeries (coeff C constantCoeff)
...
-- FIXME: Is this really what we're meant to write?
abbrev X : α⸨X⸩ := single 1 1

variable {α : Type*} [Semiring α] (φ φ' : α⸨X⸩)
#check (φ / X : LaurentSeries α)
abbrev bla : LaurentSeries α := φ / X

/-
  If `φ = a₀ + a₁ * X + a₂ * X^2 + ⋯`, then `φ * X + a₀ = φ`
-/
lemma div_X_mul_add : (φ / X * X : α⸨X⸩) = φ := by
  sorry
...
open LaurentSeries -- TODO: What's the 'normal' way to directly construct a Laurent series?
open PowerSeries (constantCoeff)
...
attribute [-simp] HahnSeries.ofPowerSeries_X

#check LaurentSeries.div

/-
  `A.shift = 2 * A + (1 - X)⁻¹`
-/
theorem left_eq_right : A / X = 2 * A + (1 - X)⁻¹ :=
  calc
    _ = mk (fun n ↦ α (n + 1)) - α 0 := by
      rw [A]
      ext n
      simp
    _ = mk (fun n ↦ 2 * (α n) + 1) := rfl
    _ = 2 * A + (1 - X)⁻¹ := sorry
